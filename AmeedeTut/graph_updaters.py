...
def get_horizontal_line_to_graph(axes, function, x, width, color):
    result = VGroup()
    line = DashedLine(
        axes.c2p(0, function.underlying_function(x)),
        axes.c2p(x, function.underlying_function(x)),
        stroke_width = width,
        stroke_color = color
    )
    dot = Dot().set_color(
    ...
    def construct(self):
        ...
        moving_h_line = always_redraw(
            lambda : get_horizontal_line_to_graph(
                plane2, func2, k.get_value(), 4, YELLOW
            )
        )
...
class AdditiveFunctions(Scene):
    def construct(self):
        ...
        for k in np.arange(0.2, 2.1, 0.2):
            line1 = DashedLine(
                axes.c2p(k, 0),
                axes.c2p(k, func1.underlying_function(k)),
                stroke_color = YELLOW,
                stroke_width = 5
            )
            ...
            line2 = DashedLine(
                axes.c2p(k, 0),
                axes.c2p(k, func2.underlying_function(k)),
                stroke_color = GREEN,
                stroke_width = 7
            )
            ...
            line3 = Line(
                axes.c2p(k, 0),
                axes.c2p(k, func3.underlying_function(k)),
        ...
        area1 = axes.get_riemann_rectangles(
            graph = func1, x_range = [0, 2], dx = 0.1,
            color = [BLUE, GREEN]
        )
        area2 = axes.get_riemann_rectangles(
            graph = func2, x_range = [0, 2], dx = 0.1,
            color = [YELLOW, PURPLE]
        )
        ...
        self.play(area2.animate.set_opacity(0.5))
        self.wait()
        for k in range(20):
            self.play(area2[k].animate.shift(
                UP * area1[k].get_height()
            ))
...
def get_arc_lines_on_func(graph, plane, dx = 1,
                          line_color = WHITE,
                          line_width = 1,
                          x_min = None,
                          x_max = None):
    ...
    for x, color in zip(x_range, colors):
        p1 = Dot().scale(0.7).move_to(
            plane.i2gp(x, graph)
        )
        p2 = Dot().scale(0.7).move_to(
            plane.i2gp(x + dx, graph)
        )
        dots.add(p1, p2)
...
class ArcLength(Scene):
    def construct(self):
        ...
        dx = always_redraw(
            lambda : DashedLine(
                axes.c2p(2, graph.underlying_function(2)),
                axes.c2p(2 + dist.get_value(), graph.underlying_function(2)),
                stroke_color = GREEN
            )
        )
        
        dx_brace = always_redraw(
            lambda : Brace(dx).next_to(dx, DOWN, buff = 0.1)
        )
        dx_text = always_redraw(
            lambda : MathTex('dx').set(width = 0.3)
            .next_to(dx_brace, DOWN, buff = 0)
        )
        
        dy = always_redraw(
            lambda : DashedLine(
                axes.c2p(2 + dist.get_value(), 
                         graph.underlying_function(2 + dist.get_value())),
                axes.c2p(2 + dist.get_value(),
                         graph.underlying_function(2)),
                stroke_color = GREEN
            )
        )
        
        dy_brace = always_redraw(
            lambda : Brace(dy, direction = RIGHT)
            .next_to(dy, RIGHT, buff = 0.1)
        )
        dy_text = always_redraw(
            lambda : MathTex('dy').set(width = 0.3)
            .next_to(dy_brace, RIGHT, buff = 0)
        )
        ...
        demo_mobjects = VGroup(
            dx, dx_brace, dx_text, dy, dy_brace,
            dy_text, dl, dl_brace, dl_text
        )
        
        
        helper_text = (
            Tex("$dL$ approximates curve as $dx$ approaches $0$")
            .set(width = 6)
            .to_edge(UR, buff = 0.2)
        )
        line1 = MathTex("{dL}^2 = {dx}^2 + {dy}^2")
        line2 = MathTex(
            "{dL}^2 = {dx}^2 \\left( 1 + \\left( \\frac{dy}{dx} \\right) ^ 2 \\right)"
        )
        line3 = MathTex(
        ...
        line_integral = always_redraw(
            lambda : get_arc_lines_on_func(
                graph,
                axes,
                dx_tracker.get_value(),
                line_color = RED,
                line_width = 5,
                x_min = -1,
                x_max = 4
        ...
        self.play(Create(dl))
        self.add(
            dx_brace, dx_text, dy_brace, dy_text,
            dl_brace, dl_text
        )
