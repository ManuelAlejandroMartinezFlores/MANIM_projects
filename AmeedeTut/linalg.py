...
class Matrix(LinearTransformationScene):
    ...
    def construct(self):
        ...
        text = always_redraw(
            lambda :
                Tex('Det(A)').set(width = 0.7).move_to(unit_square.get_center())
        )
        ...
        self.add_transformable_mobject(vect, unit_square, rect, circ)
...
class AddVector(Scene):
    def construct(self):
        ...
        vect1_name = MathTex(
            '\\vec{v}'
        ).next_to(vect1, RIGHT, buff=0.2).set_color(YELLOW)
        ...
        vect2_name = MathTex(
            '\\vec{w}'
        ).next_to(vect2, LEFT, buff=0.1).set_color(RED)
        ...
        stuff = VGroup(plane, vect1,
                       vect2, vect3, 
                       vect4, vect1_name,
                       vect2_name, vect4_name)
        ...
        self.wait()
        self.play(GrowFromPoint(vect1,
                                point = vect1.get_start()),
                  Write(vect1_name), run_time = 2)
        self.wait()
        self.play(GrowFromPoint(vect2,
                                point = vect2.get_start()),
        ...
        self.add(box)
        self.wait()
        self.play(stuff.animate.move_to(box.get_center()).set(width = 1.2), run_time = 1)
        self.wait()
